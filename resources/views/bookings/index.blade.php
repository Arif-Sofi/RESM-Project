        ...
        <div class="w-full max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div x-data="roomBookingFlow()">
                ...
                <div class="mb-8">
                    ...
                    <x-secondary-button class="ms-2" x-on:click.prevent="$dispatch('open-modal', 'date-booking-flow-modal')">
                        {{ __('messages.date') }}
                    </x-secondary-button>
                </div>
                ...
                <div x-show="showRoomBookingFlow" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                    ...
                    <form method="POST" action="{{ route('bookings.store') }}" x-show="currentStep === 2 || currentStep === 3">
                        ...
                        <div x-show="currentStep === 2">
                            ...
                            <div class="mb-6 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-700 p-4 rounded-lg bg-gray-50 dark:bg-gray-900">
                                <template x-if="previousBookings.length > 0">
                                    <ul class="space-y-2">
                                        <template x-for="booking in previousBookings" :key="booking.id">
                                            <li class="text-sm text-gray-700 dark:text-gray-300 p-2 bg-white dark:bg-gray-800 rounded border">
                                ...
                                <template x-if="previousBookings.length === 0">
                                    <p class="text-sm text-gray-500 text-center py-4">{{ __('No previous bookings found for this room.') }}</p>
                                </template>
                            </div>
                            ...
                            <div x-show="clashDetected" class="mb-4 p-3 bg-red-100 dark:bg-red-900 border border-red-300 dark:border-red-700 rounded-lg">
                        ...
                        <div x-show="currentStep === 3">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea id="equipment_needed" name="equipment_needed" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" x-model="equipmentNeeded"></textarea>
                                </div>
                                <div>
                                    <x-input-label for="purpose" :value="__('Purpose')" />
                                    <textarea id="purpose" name="purpose" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" x-model="purpose" required></textarea>
                                </div>
                            </div>
